...
echo 'deb http://download.opensuse.org/repositories/network:/osmocom:/latest/Debian_8.0 ./' > /target/etc/apt/sources.list.d/opensuse_osmocom.list
...
in-target apt-key add /var/opt/pubkey.gpg
rm /target/var/opt/pubkey.gpg
wget -qO /target/var/opt/Release.key http://download.opensuse.org/repositories/network:/osmocom:/latest/Debian_8.0/Release.key
